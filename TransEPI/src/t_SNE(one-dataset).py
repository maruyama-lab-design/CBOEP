...
for research_type in research_type_list:
    ...
    for file in file_list:
        with open(file, mode='rb') as f:
            data = pickle.load(f)
            print(data)
            data = [f"positive" if i == [1.0] else f"negative" for i in data]
            labels += data
...
plt.legend()
plt.axis('off')
plt.savefig(os.path.join(outdir, f"{research_name}_{research_type_list[0]}.png"))