...
def train_transformer_model(
    ...
    for epoch_idx in range(num_epoch):
        ...
        torch.save({
            ...
            "optimizer_state_dict": optimizer.state_dict(),
            "scheduler_state_dict": scheduler.state_dict()
            }, os.path.join(outdir, f"epoch{epoch_idx}.pt"))
        # ______
    ...
    best_model_path = os.path.join(outdir, f"epoch{best_epoch}.pt")
    ...
    np.savetxt(
            os.path.join(outdir, f"{cl}_prediction.txt"),
            np.concatenate((
...
if __name__ == "__main__":
    ...
    cl_list = config["data_opts"]["cell_type_list"]
    config["data_opts"].pop("cell_type_list")
    ...
    for cl in cl_list:
        config["data_opts"]["datasets"] = glob.glob(os.path.join(os.path.dirname(__file__), "..", "data", dataname, datatype, f"{cl}*.tsv"))
