...
def convert_TargetFinder_to_BENGI_format(filename):
    ...
    csv_df = pd.read_csv(filename, index_col=None, header=None, names=BENGI_columns)
...
def convert_TargetFinder_to_BENGI_format2(filename):
    # csv_df = pd.read_table(filename, header=None, index_col=None, names=BENGI_columns)
    csv_df = pd.read_table(filename, index_col=None, header=None, names=BENGI_columns, sep="\t")
...
for filename in glob.glob(os.path.join(__file__, "..", "maxflow", "*.tsv")):
    convert_TargetFinder_to_BENGI_format2(filename)
