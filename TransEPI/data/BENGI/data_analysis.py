...
def data_analysis(excel_path, data_dir):
    ...
    csv_df.to_csv(os.path.join(data_dir, "data_analysis.csv"), sep="\t", index=False)
    with pd.ExcelWriter(excel_path, mode="a") as writer:
        csv_df.to_excel(writer, sheet_name=f"{os.path.basename(data_dir)}_BENGI")


data_analysis(os.path.join(".", "data_analysis.xlsx"), os.path.join(".", "constrained(all_regions_have_pos_pair)"))
