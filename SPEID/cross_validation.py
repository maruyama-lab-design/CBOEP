...
def get_name2seq(args):
	...
	key_cash = ""
	with open(f"SPEID/sequence/{args.cell_line}/enhancers.fa") as f_enh:
	...
	with open(f"SPEID/sequence/{args.cell_line}/promoters.fa") as f_prm:
...
def get_chr2dataset(args):
	...
	df = pd.read_csv(f"training_data/{args.dataset}/{args.cell_line}_train.csv")
...
def chromosomal_cv(args):
	...
	for fold, test_chrom in enumerate(test_chroms):
		...
		modelCheckpoint = ModelCheckpoint(
			filepath = f'SPEID/model/{args.cell_line}_{fold+1}.h5',
			monitor='loss',
			verbose=1,
			save_best_only=True,
		...
		model.load_weights(f'SPEID/model/{args.cell_line}_{fold+1}.h5')
		print("testing...")
		...
		for dataset in ["new"]:
			for epochs in [128]:
				for batch_size in [100]:
					...
					args.output_dir = f"result\\SPEID\\{args.cell_line}\\{args.dataset}\\epochs_{args.epochs}%batch_size_{args.batch_size}"
