...
def make_prob_line_graph(
    ...
    for _, (indir, label) in enumerate(zip(indirs, labels)):
        ...
        plt.plot(np.arange(n_bins), actual_positive_ratio_by_bin.values(), label=label, color=colors[_], linewidth=0.5, marker=markers[_], markersize=3, alpha=0.6)
        plt.plot(np.arange(n_bins), actual_negative_ratio_by_bin.values(), color=colors[_], linewidth=0.5, marker=markers[_], markersize=3, alpha=0.6)

    
    plt.axhspan(0, 1, color='red', alpha=0.05) # 背景色
    plt.axhspan(-1, 0, color='blue', alpha=0.05) # 背景色
...
def make_result_bar(
        ...
        for j, (indir, label) in enumerate(zip(indirs, labels)):
            if n_fold == -1:
                ...
                if os.path.exists(infile):
                    ...
                    if i == 0:
                        plt.bar(x, score, width=width*0.90, label=label, align="center", color=colors[j], linewidth=0.4, edgecolor="black", capsize=3)
                    else:
                        plt.bar(x, score, width=width*0.90, align="center", color=colors[j], linewidth=0.4, edgecolor="black", capsize=3)
                else:
                    score = 0
            else:
                ...
                for fold_idx in range(1, n_fold+1):
                    ...
                    if len(glob.glob(f"{indir}/*{test_cell}_fold{fold_idx}.txt")) == 1:
                        infile = glob.glob(f"{indir}/*{test_cell}_fold{fold_idx}.txt")[0]
                    elif len(glob.glob(f"{indir}/*{test_cell}_fold{fold_idx}.csv")) == 1:
                        infile = glob.glob(f"{indir}/*{test_cell}_fold{fold_idx}.csv")[0]
                ...
                yerr = np.std(score_list) / np.sqrt(n_fold)
                if i == 0:
                    plt.bar(x, score, width=width*0.90, label=label, align="center", color=colors[j], yerr=yerr, linewidth=0.4, edgecolor="black", capsize=3)
                else:
                    plt.bar(x, score, width=width*0.90, align="center", color=colors[j], yerr=yerr, linewidth=0.4, edgecolor="black", capsize=3)
            print(f"{label} {test_cell}: {score} ({metric})")
    ...
    for j, label in enumerate(labels):
        yerr = np.std(average_score[label]) / np.sqrt(len(test_cells))
        plt.bar(x, np.mean(average_score[label]), width=width*0.90, align="center", color=colors[j], yerr=yerr, linewidth=0.4, edgecolor="black", capsize=3)
        if j == len(labels)//2:
            x_ticks.append(x)
        x += width
        # print(f"{label}: {np.mean(average_score[label])} +- {yerr} ({metric})")
        print(f"{label} {test_cell}: {np.mean(average_score[label])} +- {yerr} ({metric})")
...
def make_pr_curve(
    ...
    for _, (indir, label) in enumerate(zip(indirs, labels)):
        ...
        # さらに各thersholdに相当する点をプロット
        for threshold, marker in marker_by_threshold.items():
            # threshold未満と以上でprobを0と1に分け、predとする
...
if __name__ == "__main__":
    ...
    for metric in ["Balanced accuracy", "Recall", "Specificity", "AUC", "AUPR", "AUPR-ratio", "F1", "MCC", "precision"]:
        ...
        make_reuslt_bar(
            ...
            outfile=f"./bar/{metric}/cv-TransEPI_wo-mask_BGvsMFvsGS.png",
            metric=metric,
            test_cells=["GM12878", "HeLa-S3", "HMEC", "IMR90", "K562", "NHEK"],
