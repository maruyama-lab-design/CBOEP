...
def test(classifier, df, use_window, test_chroms, fold_idx, pred_dir):
	...
	# output_path = os.path.join(args.outdir, f"train_{args.train_cell}", f"test_{args.test_cell}_fold{fold_idx+1}.csv")
	output_path = os.path.join(pred_dir, f"fold{fold_idx+1}.csv")
...
if __name__ == "__main__":
	...
	# train_df = pd.read_csv(os.path.join(args.indir, f"{args.train_cell}.csv"))
	train_df = pd.read_csv(args.train_EPI)
	# test_df = pd.read_csv(os.path.join(args.indir, f"{args.test_cell}.csv"))
	test_df = pd.read_csv(args.test_EPI)
	...
	for fold_idx in range(len(train_chroms_dict)):
		args.train_chroms = train_chroms_dict[f"fold{fold_idx+1}"]
		args.test_chroms = test_chroms_dict[f"fold{fold_idx+1}"]
		classifier = train(
			df=train_df, use_window=args.use_window, train_chroms=args.train_chroms,
			gbdt_tree=config["model_opt"]["gbdt_tree"],
			gbdt_depth=config["model_opt"]["gbdt_depth"],
			gbdt_lr=config["model_opt"]["gbdt_alpha"]
		)
		test(classifier=classifier, df=test_df, use_window=args.use_window, test_chroms=args.test_chroms, fold_idx=fold_idx, pred_dir=args.pred_dir)
