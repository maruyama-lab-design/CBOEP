...
fig_root = "D:\\ylwrv\\Koga_code\\結果表示用\\TargetFinder(cl_wise)\\fig"
...
def make_heatmap(dataname, metric):
	...
	for i, cell_type in enumerate(cell_type_list):
		for j, datatype in enumerate(datatype_list):
			filenames = glob.glob(os.path.join(os.path.dirname(__file__), f"{dataname}_{datatype}", f"GM12878-{cell_type}*.csv"))
			# if len(filenames) == 0:
			# 	hist_list[i][j] = 
	...
	df = pd.DataFrame(data=hist_list, index=cell_type_list, columns=datatype_list)
	plt.figure()
	if metric == "MCC":
...
def make_hist(dataname, metric):
	...
	plt.tight_layout()
	plt.savefig(os.path.join(fig_root, "hist", f"{metric},tool=TargetFinder,dataset={dataname},train_cl=GM12878"))
