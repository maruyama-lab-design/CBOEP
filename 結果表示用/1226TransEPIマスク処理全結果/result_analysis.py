...
def make_bar(dataname, metric):
	...
	datatype_list = ["original", "maxflow_2500000", "maxflow_5000000", "maxflow_10000000", "maxflow_9999999999"]
	if dataname == "BENGI":
		column_list = ["$D_{BG}^{-}$", "$N_{2.5M}({D_{BG}^{+}})$", "$N_{5.0M}({D_{BG}^{+}})$", "$N_{10M}({D_{BG}^{+}})$", "$N_{inf}({D_{BG}^{+}})$"]
		column_list = ["BG", "NIMF($d_{\max}=$2.5M)", "NIMF($d_{max}=$5M)", "NIMF($d_{max}=$10M)", "NIMF($d_{max}={\infty}$)"]
	else:
		column_list = ["$D_{TF}^{-}$", "$N_{2.5M}({D_{TF}^{+}})$", "$N_{5.0M}({D_{TF}^{+}})$", "$N_{10M}({D_{TF}^{+}})$", "$N_{inf}({D_{TF}^{+}})$"]
		column_list = ["TF", "NIMF($d_{max}=$2.5M)", "NIMF($d_{max}=$5M)", "NIMF($d_{max}=$10M)", "NIMF($d_{max}={\infty}$)"]
		cell_type_list = ["GM12878", "HeLa", "IMR90", "K562", "NHEK"]
	...
	for train_cell_type in cell_type_list:
		...
		plt.legend(ncol=len(column_list)-2, loc="upper center")
