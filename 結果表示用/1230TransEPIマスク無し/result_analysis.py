...
def make_hist(dataname, metric):
	...
	for i, cell_type in enumerate(cell_type_list):
		...
		plt.subplot(num)
		for j, datatype in enumerate(datatype_list):
			filenames = glob.glob(os.path.join(os.path.dirname(__file__), "result", f"{dataname}", f"{dataname}_{datatype}_{cell_type}_noScheduler(lr=0.0001)_noMSE", f"GM12878-{cell_type}*.txt"))
			if filenames == []:
				print(os.path.join(os.path.dirname(__file__), "result", f"{dataname}", f"{dataname}_{datatype}_{cell_type}_noScheduler(lr=0.0001)_noMSE", f"GM12878-{cell_type}*.txt"))
				continue
			# if len(filenames) == 0:
			# 	hist_list[i][j] =
			df = None
