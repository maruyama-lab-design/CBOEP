...
def classifier_preprocess(args, d2v):
	...
	train_df = pd.read_csv(train_path, usecols=["enhancer_chrom", "enhancer_name", "enhancer_name", "promoter_name", "label"])
	data_list = []
	...
	df.to_csv(os.path.join(os.path.dirname(__file__), "tmp.csv"), index=False)
	return df
...
def chromosomal_cv(args, df):
	...
	for fold, test_chrom in enumerate(test_chroms):
		...
		x_train = train_df.drop(columns=["enhancer_chrom", "label"]).values
		...
		x_test = test_df.drop(columns=["enhancer_chrom", "label"]).values
...
def ep2vec_stage2(args):
	...
	input_dir = os.path.join(os.path.dirname(__file__), "d2v", args.cell_line, args.way_of_kmer)
