...
def hold_out(args):
	...
	for fold, test_chrom in enumerate(test_chroms):
		...
		modelCheckpoint = ModelCheckpoint(		
			filepath = os.path.join(os.path.dirname(__file__), "model", f"{args.cell_line}_holdout.h5"),
			monitor='loss',
			verbose=1,
			save_best_only=True,
			save_weights_only=False,
		...
		model.fit(
			...
			# class_weight={1: 20, 0:1},
			callbacks=[modelCheckpoint, earlystopper, csv_logger],
			validation_split=([X_valid_enh, X_valid_prm], [Y_valid])
		)

		model.load_weights(os.path.join(os.path.dirname(__file__), "model", f"{args.cell_line}_holdout.h5"))
		print("testing...")
		y_pred = model.predict(
			[X_test_enh, X_test_prm],
			batch_size=args.batch_size,
