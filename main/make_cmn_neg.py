...
def make_all_pair(dataname, cell_type):
    if os.path.exists(os.path.join(os.path.dirname(__file__), "pair_data", dataname, "original", f"{cell_type}.csv")) == False:
        return
    org_df = pd.read_csv(os.path.join(os.path.dirname(__file__), "pair_data", dataname, "original", f"{cell_type}.csv"))
    ...
    os.makedirs(os.path.join(os.path.dirname(__file__), "pair_data", dataname, "cmn_test_pair"), exist_ok=True)
    all_pair_df[csv_columns].to_csv(os.path.join(os.path.dirname(__file__), "pair_data", dataname, "cmn_test_pair", f"{cell_type}.csv"), index=False, sep=",")

   



for cell_type in ["GM12878"]:
